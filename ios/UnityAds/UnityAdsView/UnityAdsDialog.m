...
- (id)initWithFrame:(CGRect)frame useSpinner:(BOOL)createSpinner useLabel:(BOOL)createLabel useButton:(BOOL)createButton {
    ...
    if (self) {
      if (createSpinner) {
        int spinnerSize = 47;
        self.spinner = [[UnityAdsNativeSpinner alloc] initWithFrame:CGRectMake(12, (frame.size.height / 2) - (spinnerSize / 2), spinnerSize, spinnerSize)];
      ...
      if (createButton) {
        ...
        self.button = [[UnityAdsNativeButton alloc] initWithFrame:CGRectMake((frame.size.width / 2) - (buttonWidth / 2), frame.size.height - buttonHeight - 9, buttonWidth, buttonHeight) andBaseColors:gradientArray strokeColor:[UIColor greenColor]];
