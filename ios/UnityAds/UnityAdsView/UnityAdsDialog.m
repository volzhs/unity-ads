...
- (void)createView {
  ...
  if (self.useSpinner) {
    int spinnerSize = 47;
    self.spinner = [[UnityAdsNativeSpinner alloc] initWithFrame:CGRectMake(12, (self.frame.size.height / 2) - (spinnerSize / 2), spinnerSize, spinnerSize)];
  ...
  if (self.useButton) {
    ...
    self.button = [[UnityAdsNativeButton alloc] initWithFrame:CGRectMake((self.frame.size.width / 2) - (buttonWidth / 2), self.frame.size.height - buttonHeight - 9, buttonWidth, buttonHeight) andBaseColors:gradientArray strokeColor:[UIColor greenColor]];
