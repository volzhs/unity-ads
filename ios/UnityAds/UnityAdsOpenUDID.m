...
@interface UnityAdsOpenUDID (Private)
...
@implementation UnityAdsOpenUDID
...
//    }
	...
	// [[ this block of code has been removed, it's now handled by Unity Ads itself ]]
...
//
+ (NSString*) value {
    return [UnityAdsOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [UnityAdsOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [UnityAdsOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [UnityAdsOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
