...
- (NSString *)_queryString
{
	...
	return [NSString stringWithFormat:@"?openUdid=%@&macAddress=%@%@&iosVersion=%@&device=%@&sdkVersion=%@&gameId=%@&type=ios&connection=%@", self.md5OpenUDID, self.md5MACAddress, advertisingIdentifier, [[UIDevice currentDevice] systemVersion], self.machineName, kUnityAdsVersion, self.gameId, [self _currentConnectionType]];
}
...
{
	if ([self.delegate respondsToSelector:@selector(unityAds:completedVideoWithRewardItem:)])
		[self.delegate unityAds:self completedVideoWithRewardItem:self.rewardItem];
}
