...
public class UnityAdsCacheManifest {
	...
	public UnityAdsCacheManifest (String manifestDir) {
	...
	public ArrayList<String> getCachedCampaignIds () {
		...
		try {
			...
			Log.d(UnityAdsProperties.LOG_NAME, "Malformed manifest");
			return null;
		}
	...
	public boolean removeCampaignFromManifest (JSONObject campaign) {
		...
		try {			
			...
			Log.d(UnityAdsProperties.LOG_NAME, "Malformed cache manifest.");
			return false;
		}
		...
		if (originalVa != null) {
			for (int i = 0; i < originalVa.length(); i++) {
				try {
					...
					Log.d(UnityAdsProperties.LOG_NAME, "Couldn't get current campaign index.");
					return false;
				}
	...
	public boolean addCampaignToManifest (JSONObject campaign) {
		...
		if (!manifest.exists() || _manifestJson == null || !_manifestJson.has("va")) {
			Log.d(UnityAdsProperties.LOG_NAME, "Preparing new cache manifest");
		...
		if (_manifestJson != null && _manifestJson.has("va") && getCampaign(campaignId) == null) {
			try {
				...
				Log.d(UnityAdsProperties.LOG_NAME, "Problem creating JSON: " + e.getMessage());
				return false;
			}
		}
	...
	public boolean updateCampaignInManifest (JSONObject campaign) {
		...
		if (campaign.has("id")) {
			...
			Log.d(UnityAdsProperties.LOG_NAME, "Updating campaign: " + campaignId);
			manifestCampaign = getCampaign(campaignId);
			
			if (manifestCampaign != null) {
				try {
					if (!manifestCampaign.getString("v").equals(campaign.getString("v")) && !UnityAds.cachemanager.isFileCached(campaign.getString("v"))) {
						// TODO: Make check that the old video-file is not needed anymore by other campaigns and remove it
						// UnityAds.cachemanager.removeCachedFile(manifestCampaign.getString("v"));
						
						// TODO: Do not call cacheFile from here, caching should be done in cachemanager
						UnityAds.cachemanager.cacheFile(campaign.getString("v"), campaignId);
					}
				}
				catch (Exception e) {
					Log.d(UnityAdsProperties.LOG_NAME, "Malformed cache manifest.");
				...
				try {
					...
					Log.d(UnityAdsProperties.LOG_NAME, campaign.getString("s") + ", " + campaign.getString("v"));
				}
				catch (Exception e) {
					Log.d(UnityAdsProperties.LOG_NAME, "Malformed cache manifest.");
					return false;
	...
	private boolean readCacheManifest () {
		...
		if (manifest.exists() && manifest.canRead()) {
			try {
				...
				Log.d(UnityAdsProperties.LOG_NAME, "Problem reading cachemanifest: " + e.getMessage());
				return false;
			}
			...
			Log.d(UnityAdsProperties.LOG_NAME, "Cache manifest did not exist or couldn't be read");
		}
	...
	private boolean writeCurrentCacheManifest () {
		...
		try {
			...
			Log.d(UnityAdsProperties.LOG_NAME, "Could not re-write cachemanifest: " + e.getMessage());
		}
	...
	private File getFileForManifest () {
		return new File(_manifestDirectory + "/" + UnityAdsProperties.CACHE_MANIFEST_FILENAME);
	}
}